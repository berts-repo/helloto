...
- **Server IP**: `10.0.0.1` (local IP, e.g., 192.168.0.200)
- **WireGuard Tunnel IPs**:
  - Full tunnel: `10.0.0.0/24`
  - Mobile tunnel: `10.10.10.0/24`
- **Router Public IP**: `vpn.example.com`
- **WireGuard Ports**:
  - Main tunnel: `51820`
  - Mobile tunnel: `51821`
...
iface enp0s3 inet static
    address 10.0.0.1  # e.g., 192.168.0.200
    netmask 255.255.255.0
    gateway 192.168.0.1    # e.g., 192.168.0.1
    dns-nameservers 192.168.0.53 192.168.0.153  # e.g., 192.168.0.53 192.168.0.153
```
...
nmcli con mod "Wired connection 1" ipv4.dns "192.168.0.53 192.168.0.153"
...
PostUp = iptables -A FORWARD -i mobile -d 192.168.0.120 -j ACCEPT
PostUp = iptables -A FORWARD -o mobile -s 192.168.0.53 -j ACCEPT
PostUp = iptables -A FORWARD -o mobile -s 192.168.0.120 -j ACCEPT
PostUp = iptables -t nat -A POSTROUTING -o enp6s18 -j MASQUERADE

PostDown = iptables -D FORWARD -i mobile -d 192.168.0.53 -j ACCEPT
PostDown = iptables -D FORWARD -i mobile -d 192.168.0.120 -j ACCEPT
PostDown = iptables -D FORWARD -o mobile -s 192.168.0.53 -j ACCEPT
PostDown = iptables -D FORWARD -o mobile -s 192.168.0.120 -j ACCEPT
PostDown = iptables -t nat -D POSTROUTING -o enp6s18 -j MASQUERADE
...
Address = 10.0.0.2/32
DNS = 192.168.0.53  # Use Pi-hole for DNS
...
DNS = 192.168.0.153  # Backup Pi-hole instance

[Peer]
PublicKey = <paste server_public.key contents>
Endpoint = vpn.example.com:51821
AllowedIPs = 192.168.0.120/32, 10.10.10.0/24  # Only Jellyfin and tunnel network
PersistentKeepalive = 25
```
